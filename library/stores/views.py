        ...
        if form.is_valid():
            ...
            try:
                if Subscribers.objects.filter(email = my_model.email).first():
                    ...
                    mail_test(TO=my_model.email,SUBJECT=SUBJECT,newsletter=True,authentication=False,token=None)
                    my_model.save()
                    return redirect('/')
...
def admin_local(request):
  if is_admin(request.user):
    ...
    return render(request,'stores/admin.html',{'is_superuser':False})

def orders_view(request, book_id ):
    if request.user.is_authenticated:
        profile = get_object_or_404(Profile, user=request.user)#takes the current logged in user
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                new_order = Order(
                    ...
                    user = profile #stores the user to the order 
                )
                new_order.save()
                return redirect('/')
